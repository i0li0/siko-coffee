  ...
  async run() {
    ...
    new sst.aws.Nextjs('Web', {
      ...
      server: {
        ...
        //    ウォーム p50 73.8 → **49.0 ms** / p90 281 → **83 ms**
        //    コールド p50 2,041 → **1,073 ms（−47%）**  ← 効いたのは主にここ
        //
        // 💰 「GB-秒課金なので費用は相殺される」は**言い過ぎだった**。変更前のコールド率(15.6%)で
        // 加重平均すると 0.381 → 0.417 GB-秒＝**約 +10%**（実行時間の短縮が2倍のメモリを
...
//     2048MB で約1.16vCPU。実測は **コールド p50 2,041→1,073ms（−47%）/ ウォーム p50 73.8→49.0ms**。
