...
export default $config({
  ...
  async run() {
    ...
    //
    // ✅ **2026-08-02（13-4 の DNS 切替後）に 'production' を足した＝5-3。**
    //
    // 🔴🔴 **かつてここには「13 で DNS を切り替えたあとに足せば安全」と書いてあったが、
    //    その理由づけは誤りだった**（2026-08-02 に発見）。理由は「両方が同時に回ると
    //    `instagram-refresh` が競合する」で、**DNS 切替がそれを解消するかのように読めた**。
    //    しかし **DNS 切替は Vercel の cron を止めない。** `vercel.json` の crons 4本は
    //    soak 中ずっと生きている（ロールバック先を壊さないため Vercel は触らない方針）。
    //    ＝ **二重実行は 15（Vercel 解約）まで続く。** 判断すべきだったのは
    //    「切替が済んだか」ではなく **「二重実行を許容できるか」** だった。
    //
    // ✅ **採った対処: AWS 側の発火時刻を Vercel と重ならない位置へずらした**（下の CRON_JOBS）。
    //    二重実行そのものは残るが、**危険なのは同時実行（レース）であって重複ではない**。
    //    Vercel 側を触らずに済むので**ロールバック先も壊れない**。
    //    📌 なぜ「重複は許せてレースは許せない」か: 3本は idempotent と分かっている
    //       （release-reservations は reconcile 型で 2026-07-28 に手動実行で実証済み、
    //       cleanup-pending は ConditionalCheckFailedException を skip する設計）。
    //       一方 **instagram-refresh は共有の DynamoDB から同じトークンを読んで
    //       `PutCommand` で無条件上書きする**ので、同時に走ったときの結果は
    //       **Instagram 側の仕様に依存する＝こちらで確かめていない**。
    //       確かめていないものに賭けないための、時刻ずらしである。
    const CRON_STAGES = ['dev', 'production']

    // 📌 スケジュールは **EventBridge Scheduler の書式**（cron は6フィールドで、
    //    day-of-month と day-of-week のどちらかを `?` にする）。
    //
    // 🔴🔴 **2026-08-02（5-3）に、全4本を `vercel.json` と重ならない時刻へずらした。**
    //    かつては「時刻は UTC のまま vercel.json と揃えてあるので移行の前後で発火時刻は
    //    変わらない」ことが**利点**だった。soak で両方が本番を担うようになった今、
    //    **同じ時刻に揃っていることは「毎日レースが起きる」という意味に変わった**。
    //    ＝ **同じ設計判断が、前提が変わると逆の意味を持つ。**
    //
    // 🔴🔴 **ずらし幅を決めるのは「指定時刻」ではなく「Vercel の発火窓」である。**
    //    Vercel Hobby の cron は **1時間の flexible time window** を持つ
    //    （ダッシュボードの明記「Cron jobs on Hobby have a flexible time window of 1-hour」＋
    //    実測: `instagram-refresh` は 00:00 指定に対し **00:21 に発火**した）。
    //    ＝ 「18:00 のジョブ」は実際には **18:00〜18:59 のどこか**で走る。
    //    **最初 30分ずらす案を書いたが、それでは窓の内側で無意味だった。**
    //
    //    | ルート | Vercel の**発火窓**（触らない） | AWS（ここ） | 重なるか |
    //    |---|---|---|---|
    //    | instagram-refresh   | 毎月1日 00:00〜01:00 | **毎週日曜 03:30** | ✅ 重ならない |
    //    | cleanup-pending     | 毎日 18:00〜19:00    | **毎日 20:00**     | ✅ 重ならない |
    //    | po-timeouts         | 毎日 18:20〜19:20    | **毎日 20:20**     | ✅ 重ならない |
    //    | release-reservations| 毎日 18:10〜19:10    | **毎時 :05,:15,…** | 🔴 **重なりうる** |
    //
    //    日次3本の窓の和は **18:00〜19:20** なので、AWS 側を **20:00 以降**へ寄せた。
    //
    // 🔴 **release-reservations だけは重なりを消せない。** 10分間隔で回す以上、
    //    18:10〜19:10 の窓には必ず入る。ここは**重なりを許容する**判断で、根拠は2つ:
    //    ① reconcile 型で **idempotent であることを実測済み**（2026-07-28 に手動実行で
    //       `reservedG 200→0` を確認。同時に走っても実際の予約から再計算するので収束する）。
    //    ② そもそも **Vercel 側のこの1本は実行されていなかった**（8 を作った動機そのもの）。
    //    ＝ **確かめていないものには賭けず、確かめたものだけ許容する**という線引き。
    //
    // 🔴 **`rate(10 minutes)` をやめた理由**: rate は**作成時刻からの相対**で発火するので
    //    位相が不定＝いつ走るかを**設計時に決められない**。`cron(5/10 …)` なら壁時計に固定され、
    //    少なくとも**いつ走るかが確定する**（10分間隔という 8 の狙いは維持）。
    //    📌 AWS 公式の定義: `/` は増分で、**フィールドの先頭値から刻む**
    //       （docs の例「Minutes に `1/10` で 1,11,21,31…」）＝ `5/10` は :05,:15,:25,:35,:45,:55。
    // 📌 EventBridge Scheduler の発火精度は **60秒**（docs 明記）＝ 上の「重ならない」は
    //    分単位で1時間以上離してあるので精度の揺れでは崩れない。
    // 🔴 **15（Vercel 解約）後にこのずらしは不要になる**が、戻す必要も無い
    //    （戻す作業自体がリスク）。16 の撤去リストには入れていない。
    const CRON_JOBS = [
      {
        ...
        //       これ以上詰めても意味はない。
        // 🔴 **日曜 00:00 → 日曜 03:30 UTC へずらした（5-3）。これが今回いちばん大事な1行。**
        //    Vercel は毎月1日 00:00 指定＝ **発火窓は 00:00〜01:00**。元のままだと
        //    **1日が日曜の月に真正面から衝突**した（両者が共有の DynamoDB から同じトークンを
        //    読んで `PutCommand` で無条件上書きする＝結果は Instagram 側の仕様次第で、
        //    **こちらで確かめていない**）。時刻帯そのものを窓の外へ出したので
        //    **曜日と日付がどう重なっても衝突しない**。＝ 日曜 03:30 UTC ＝ 日曜 12:30 JST。
      ...
      },
      {
        ...
        // 🔴 18:00 → **20:00**（Vercel の発火窓 18:00〜19:00 の外へ・5-3）
        schedule: 'cron(0 20 * * ? *)',
      },
      {
        ...
        // 🔴 **`rate(10 minutes)` → `cron(5/10 …)`（5-3）。** rate は作成時刻からの相対で
        //    位相が不定＝ Vercel の 18:10 と重なるかを設計時に決められない。
        //    `5/10` は :05,:15,:25,:35,:45,:55 に固定され、**:10 に当たらない**。
        //    10分間隔という狙いは変えていない。
        schedule: 'cron(5/10 * * * ? *)',
      },
      {
...
//     🔴 **「Vercel 本番の30本と突合済みで過不足なし」は誤りだったので消した**（#125・#128）。
//        あれは**値が Vercel に在ることの確認**であって**配線の確認ではなかった**（上の 100 行目付近）。
//        実際に `sst.config.ts` へ配線されていた秘密は**7本だけ**で、着手前の production の
//        secret は **0本**だった。**投入対象は 17本（必須7＋任意10）に確定済み**＝ 2026-08-01 に
//        17/17 投入完了。判断軸は「Vercel にあるか」ではなく**「消費側が何を先に読むか」**。
//     ✅ **TOTP の再登録も完了（2026-08-02）**＝ 13 に残っていた手作業は無い。
//        `siko-coffee-config` の `totp_secret` に値があり、**`totp_last_step` に step が
//        記録されている**（`setLastStep` は検証成功時にしか書かれない＝実際に通った証拠）。
