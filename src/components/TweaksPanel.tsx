...
export default function TweaksPanel() {
  ...
  const accents: Record<string, string> = { gold: '#B8BEC8', silver: '#a0a09a', rose: '#c87878' }

  if (!open) return null

  return (
    <div className="fixed bottom-20 right-[22px] z-[500] w-[218px]
      bg-[rgba(10,10,22,0.96)] border border-[rgba(184,190,200,0.18)]
      rounded-lg p-[18px] backdrop-blur-[14px]
      font-sans text-[12px] text-[rgba(232,234,238,0.45)]">
      <div className="flex justify-between items-center font-serif text-[12.5px]
        tracking-[0.15em] text-[#B8BEC8] mb-[14px]">
        <span>Tweaks</span>
        <button onClick={close} className="bg-transparent border-none cursor-pointer text-[16px]
          leading-none text-[rgba(232,234,238,0.45)]">×</button>
      </div>

      <div className="mb-[13px]">
        <span className="block text-[10px] text-[rgba(184,190,200,0.5)] tracking-[0.1em]
          uppercase mb-[7px]">Opening speed</span>
        <div className="flex gap-1">
          {Object.entries(speeds).map(([k, v]) => (
            <button key={k}
              onClick={() => window._setSpeed?.(v)}
              className="flex-1 bg-[rgba(232,234,238,0.04)] border border-[rgba(232,234,238,0.09)]
                text-[rgba(232,234,238,0.35)] rounded px-[3px] py-[5px]
      ...
      <div className="mb-[13px]">
        ...
        <input type="range" min="0.02" max="0.18" step="0.01" defaultValue="0.09"
          ...
          className="w-full h-[2px] appearance-none bg-[rgba(184,190,200,0.18)] rounded cursor-pointer" />
      </div>
