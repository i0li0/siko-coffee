...
>
> 📋 **「Pour Over が終わったとき何が残るのか」は
> [pour-over-leftovers.md](./pour-over-leftovers.md)（積み残し一覧）**。
> 本編の残り・意図的に見送った判断・推奨タスク・スコープ外を**1か所に集約**した索引で、
> 各項目に**現況の実測と確認コマンド**が付いている。**新しい積み残しが出たらそこに足す。**
...
#### ~~🔑 販売再開の手順（順番が重要）~~ ✅ **完了（2026-08-21）— この手順はもう実行しない**

> 🔴🔴 **以下は 2026-07-26 に書いた手順で、投入先が Vercel のまま腐っている。**
> 13（2026-08-02）で本番は AWS へ移り、15 の ②③④（2026-08-21）で**決済は再開済み**。
> **いま実行すると、まもなく解約する Vercel に `sk_live_` を投入することになる。**
>
> **正しい手順と実測は [`pour-over-15-16-runbook.md`](pour-over-15-16-runbook.md) §3 が正本。**
> 実際との違いは3つ: 投入先が **`sst secret set --stage production`**、本数が **3本**
> （＋ `STRIPE_WEBHOOK_SECRET`。無いと**決済は成立するのに注文が確定しない**）、
> そして事前に **`sst.config.ts` への配線 PR** が要った（宣言 ≠ 配線）。

1. ~~パスワードマネージャから `sk_live_...Nk7B` を取り出し、Vercel 本番の `STRIPE_SECRET_KEY` を**更新**~~
2. ~~`PAYMENTS_ENABLED=true` を追加~~
3. ~~再デプロイ~~（**静的生成される `/shop`・`/shop/product/[key]` の停止表示はこれをしないと消えない** … ここだけは今も真）
...
**進捗（2026-08-02 時点）: 18 / 21。** 第0群 4/4・第1群 4/4・**第2群 5/5**・
**9.5・10・11・12・13 完了**。次は **14（soak）**。

🟢🟢 **13 完了＝本番トラフィックは AWS（CloudFront `d38zi1bm4zf9e3`）で稼働中。**
切替は **`2026-08-02T19:39:15Z`**（Route53 の `SubmittedAt` が正本）。
✅ 12 の apex 308 / HSTS / `domain` も**ここで初めて実測して合格**（dev では原理的に動かなかった）。
✅ 5 の production 再確認（`AWS_IAM`・直叩き 403）、6 の WAF 初回実測（`/admin` 202）も合格。
⏳ **残る後始末**: 5-2（Vercel に `AVATAR_*` ＋IAM）・5-3（`CRON_STAGES` に production）。
**積み残しの全体像は `docs/pour-over-leftovers.md`。**

🔴 **切替で分かったこと（教訓40）**: Route53 で **www の型を変える操作は `UPSERT` では拒否される**
（`InvalidChangeBatch`）。同一バッチで **`DELETE`（旧 CNAME）＋ `CREATE`（新 A ALIAS）** が要る。
**切り戻し（§6）も同じ穴を持っていた**ので同日中に修正した＝ 型が変わる場面は切替と切り戻しの2回ある。
🔑 **切替の成否はステータスコードでは判定できない**（Vercel も AWS も同じ 200/307/308 を返す）。
**両者で必ず異なる値**を1つ見ること＝実測は `x-amz-cf-pop: KIX56-P4`。
...
✅ **この設計は実際に回収された**: `--resolve` での事前検証が実 DNS と完全に一致し、
「デプロイ → 検証 → 切替」の3段が設計どおり機能した（検証で落ちれば切り替えなければよい）。
...
| # | 作業 | 期待できる結果 |
|---|---|---|
| 13 | **production ステージへデプロイ → 検証 → DNS 切替**<br>📕 **当日の手順書は `docs/pour-over-13-runbook.md`** | ✅ **前提だった secret 投入は完了（2026-08-01・17/17）**。点検時は **0本**で、`SECRET_NAMES` の7本が1本でも欠けると `sst deploy --stage production` が落ちるため **13 はそのままでは1行目から進まなかった**。🔴 **Vercel の env は `sensitive` 型で誰も復号できない**ため「コピーする」は不可能で、**他システムからの復元12本＋両側ローテーション5本**で用意した（詳細は runbook §1）。✅ **TOTP の再登録も完了（2026-08-02）＝ 13 に残る手作業は無い**（`totp_secret` に値があり、検証成功時にしか書かれない `totp_last_step` も記録されている）<br>✅ **2・3（デプロイと切替前検証）は 2026-08-02 に完了＝残るは 4（DNS 切替）だけ**。実測値は `docs/pour-over-log.md` の同日の節<br>🔴 **前倒しで「production が main から取り残される窓」が開いた**: `ci.yml` の `matrix.stage` は `[dev]` のままなので**今 main にマージしても production には入らない**。検証済みの production は `10e8229` 時点のコード。**切替の直前にコードが動いていたら再デプロイして 3 をやり直す**<br>🔴 **DNS 切替は SST ではなく人が Route53 で行う**（12 で `dns: false` にしたため）。apex の A と www の CNAME を CloudFront への ALIAS へ UPSERT する。**切り戻しは同じ操作で Vercel へ戻すだけ＝ 11 の 60s TTL がここで効く**<br>🔴 **事前検証は CloudFront の URL ではできない**（`domain` を付けると SST が `*.cloudfront.net` を 403 にする）。`curl --resolve www.sikocoffee.com:443:<配信IP> https://www.sikocoffee.com/...` で SNI と Host を本番ドメインのまま当てる。**12 の apex 308 ＋ HSTS はここで初めて実測できる**（dev では原理的に動かない）<br>🔴 ~~シークレットは Vercel 本番の30本と突合済みで過不足なし~~ **これは誤りだったので取り消す（#125・#128）**。あれは**値が Vercel に在ることの確認**であって**配線の確認ではなかった**＝実際に `sst.config.ts` へ配線されていたのは **7本だけ**で、着手前の production の secret は **0本**だった。**投入対象は 17本（必須7＋任意10）**で、判断軸は「Vercel にあるか」ではなく**「消費側が何を先に読むか」**。なお `SITE_URL`/`NEXTAUTH_URL` にコードのフォールバック（`https://www.sikocoffee.com`）がある点と、AWSキー3本の廃止・BLOB3本の不要は今も正しい<br>🔴 **5 の確認をこのステージでもやり直すこと**（`protection` はステージごとに効くので、dev で閉じても production は別）。`AuthType: AWS_IAM` と直叩き 403 を再測する<br>🔴 **4 の積み残し②③をここで回収する**: production のバケット名で `AVATAR_UPLOAD_BUCKET` / `AVATAR_BUCKET` / `AVATAR_BASE_URL` を Vercel 本番にも入れ、Vercel の IAM ユーザーに S3 権限を足す。**それまで本番のアイコン設定は 503 のまま**（オーナー判断・2026-07-29。理由は `docs/pour-over-log.md` 教訓20）<br>🔴 **8 の cron を DNS 切替の“あと”で有効化する**: `sst.config.ts` の `CRON_STAGES` に `'production'` を足して再デプロイ。それまで production のスケジュールは DISABLED で作られており、cron は Vercel 側だけが回している。**先に有効化すると `instagram-refresh` が Vercel と二重に走り、長期トークンの更新が競合する** |
