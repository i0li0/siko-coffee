...
**進捗（2026-08-01 時点）: 13 / 21。** 第0群 4/4・第1群 4/4・**第2群 5/5（5・6・7・8・9 が完了＝第2群は消化済み）**。
🔶 **9.5 は実装済みだが未完了**（コードは入ったが、AWS 側のロール作成と CI からの実デプロイが未実施）。
このプロジェクトの完了基準は「実測で確かめた」ことなので、**14/21 に進めるのは
CI が dev へ実際にデプロイして緑になってから**。詳細は下の 9.5 の行。
...
| # | 作業 | 期待できる結果 |
|---|---|---|
| **9.5** | 🔶 **GitHub Actions ＋ OIDC で `sst deploy` を自動化（実装済み・未検証）** | **16項目の版で抜けていた**。パリティ表の項目27（push で自動デプロイ）に対応するタスクが実行順に存在しなかった。無いと 14（soak）で **main に push すると Vercel だけが更新され、本番を担う AWS は取り残される**。加えてデプロイが「npm 11 の入った特定のマシンから手打ち」に固定される。OIDC ロールにすれば静的キーも増えない（項目12と同方向） |
