...
- **最終実測日: 2026-08-13**（S-3 の判定 ＋ 15 ①の配線）。

## 全体像

| 群 | 本数 | 説明 |
|---|---|---|
| **A. Pour Over 本編の残り** | **2**（元3。✅ 14 soak 完了） | 21タスクのうち未完のもの（15・16）。これが終われば Pour Over は完了 |
| **B. 意図的に見送った技術判断** | 4 | やらないと決めたのではなく、**判断材料が揃うまで待っている**もの |
| **C. Pour Over が生んだ小さな負債** | **1**（元5・C-1/C-2/C-4/C-5 完了。残るは C-3） | 移行の過程で生まれ、まだ回収していないもの |
| **D. 推奨タスク（R-1〜R-10）** | 9 | コスト非制約の前提で挙がった改善。R-3 は不採用確定 |
| **E. スコープ外と明記したもの** | 5 | **Pour Over と混ぜないと決めた**もの。完了後に着手する |

---

## A. Pour Over 本編の残り（2本）

進捗 **19/21**。✅ **13 は完了＝本番トラフィックは AWS（CloudFront）で稼働中**
（切替 `2026-08-02T19:39:15Z`）。✅✅ **14（soak）も完了**（S-3 達成 `2026-08-13T17:55Z` 実測）。
残るは **15（決済再開＋Vercel 解約）と 16（Vercel 依存の撤去）**だけ。
...
| ~~**14**~~ | ~~**soak 期間**（Vercel を生かしたまま観測）~~ | — | ✅✅ **完了（2026-08-13）。終了条件6つすべて達成**。最後まで残った S-3 は `2026-08-05T19:53:07Z` → `2026-08-13T17:55:35Z` の **7日22時間で遷移0件**（2リージョン・負の対照 20件/12件・現在の12本すべて `OK`） |
...
| S-3 | **アラーム遷移ゼロの連続日数 ≥ 7日** | 週次 cron を1周含むため。**状態ではなく履歴**で見る（教訓42）／**2リージョン**で（教訓43） | 🟢 **維持中・唯一の残り条件**。**08-09T05:12Z 実測＝ 2リージョンの production 7本すべて OK・08-05T19:53:06Z 以降の遷移ゼロ**（`describe-alarm-history` を全本走査）。経過 **3日16時間** → 到達は **08-12T19:53Z**（残り約3.3日）。🔑 **事故のたびに起算が後ろへ動く**ので、他が揃っても引き直す |
