...
C-4 で state の平文を削除した後、残った38オブジェクトを走査したら
`eventlog/` の3本に **`SST_SECRET_` が17回**出た。**「修正後なのに平文が残っている」と一度判断した。誤りだった。**

`SST_SECRET_` は**変数名にも値の周辺にも現れる**ので、出現の有無では何も決まらない。
決め手になったのは**値だけが持つ特徴**のほうだった:

| | 値が実在した版（pre-fix） | 今回の `eventlog/` |
|---|---|---|
| `"ASIA`（一時アクセスキーの先頭） | **4〜8件** | **0件** |
| `SST_SECRET_` の出現 ÷ 名前17本 | **2〜4回/本** | **1回/本** |

＝ 入っていたのは**名前だけ**。**名前は秘密ではない。**

🔑 **規則: 「秘密が入っていないこと」を確かめるときは、
秘密の“ラベル”ではなく“中身の形”を探す。**
ラベルで探すと**偽陽性**（名前だけを見て残存と誤認）と
**偽陰性**（ラベルの無い値を見逃す）の両方が出る。
