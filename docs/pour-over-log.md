...
🔴 **main への push は必ず AWS へデプロイする。＝ マージ順そのものが実害を持つようになった。**
壊れた状態を直す PR より先に無関係な PR をマージすると、その無関係な変更が
原因であるかのような赤を生む。2026-08-01 に #116（依存の勧告対応）と #117（sharp 修正）が
まさにこの構図になり、**#117 を先に入れる必要があった**（両者はファイルが排他で
テキスト衝突はしないので、Git だけ見ていると順序の必要性に気づけない）。
📌 **PR の CI が緑でもデプロイ可否は何も保証しない**（`deploy` は `push` 限定で PR ではスキップ）。
＝ **「レビューで緑だったのに main で落ちた」は、この設計では正常**。
