...
（エージェント実行を検知したときの保護。`--non-interactive` がその場合の既定）。
...
📌 **この作業自体はエージェントには実行できない**ことも分かった。Vercel が意図的に
値を伏せているので、**オーナー本人が対話的なターミナルでやる**しかない。
手順書の 1-3 に明記した。**回避しようとしないこと**——伏せているのは保護であって障害ではない。
