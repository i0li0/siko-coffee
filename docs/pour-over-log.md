...
### 2026-08-07 — soak 中の 5xx 再発の原因が判明（Lambda 同時実行クォータ 10）と、その是正

#### 🔴🔴 症状 — アラームが2回鳴った

`siko-production-cloudfront-5xx` が **08-05T12:16Z（38.89%）** と **08-05T19:31Z（32.81%）** に発報。
🟢 **R-4（アクセスログ）のおかげで、その場で原因まで言えた**。08-02 に同じ症状で
行き止まりになった時との差は、**診断の道具が事故の前に用意されていたこと**だけである（教訓44）。

#### 原因 — `LambdaLimitExceeded`。クォータは `ap-northeast-1` だけ 1000、他は全部 10

アクセスログの `x-edge-result-type` は **164件すべて `LambdaLimitExceeded`（503）**。
実測したアカウントのクォータ（`L-B99A9384`）:

| リージョン | 同時実行 |
|---|---|
| `ap-northeast-1` | **1000** |
| `us-east-1` / `us-east-2` / `us-west-2` / `eu-west-1` / `eu-central-1` / `ap-southeast-1` / `ap-south-1` / `sa-east-1` | **10** |

`protection: 'oac-with-edge-signing'`（5）が origin-request に Lambda@Edge を付け、
SST が作るビヘイビアは **default の1本だけ**なので、この関数は
**キャッシュ MISS のたび全パスで動く**。ブラウザは1ページで静的アセットを数十本
**同時に**取りに行くため、**11 本目で throttle → CloudFront が 503**。

#### 証拠（件数まで一致・負の対照つき）

`ConcurrentExecutions` の **Maximum がちょうど 10**（＝クォータそのもの）、
`Throttles` = **105 / 47**、アクセスログの 503 も **105 / 47** で完全一致。
そして 🟢 **`Errors` は 0 ＝ 関数は落ちていない。絞られただけ**。

**発生の全リスト**（Lambda@Edge のレプリカは実行リージョン側にメトリクスを出す・JST）:

| 時刻 | リージョン | throttles | アラーム |
|---|---|---|---|
| 08-03 04:45 | us-west-2 | 108 | ✅ |
| 08-03 05:45 | us-east-2 | 109 | ✅ |
| 08-05 21:00 | us-east-2 | 7 | ✅ |
| 08-06 04:15 / 04:30 | us-east-1 | 105 / 47 | ✅ |
| 08-06 11:00 | eu-central-1 | 5 | ❌ **鳴らず** |

🔑 **これで 08-02 の「行き止まりだった3回のスパイク」のうち2回が説明できた**
（当時記録した「約109件」は us-west-2 の 108 と一致する）。
🔴 **eu-central-1 の5件はアラームが鳴っていない**（5分バケットの率が 5% を超えなかった）
＝ **アラームは「起きたこと」の下限しか見せない**。数えるならログ側で `LambdaLimitExceeded` を数える。

#### 踏んだのは誰か

08-05T12:11Z は **Googlebot（`66.249.73.x`・DFW59）が JS チャンクで 503** ＝ **SEO 影響**。
08-05T19:26Z は単一 IP のバースト（IAD55 に 635 req/20分）。
🔑 **日本のユーザーは無傷だが、それは `ap-northeast-1` だけ 1000 だった偶然による**。
海外エッジ（＝クローラーの大半）は 11 並列で落ちる。

#### 🔑 その Lambda@Edge は GET では何もしていなかった

実物は **685 バイト**で、中身はこれだけ:

```js
if (!["POST","PUT","PATCH"].includes(e.method)) return e;   // ← 即 return
// 以降は本文の sha256 を x-amz-content-sha256 に載せるだけ
```

Function URL への SigV4 署名に**本文ハッシュ**が要るから置かれている関数で、
**本文を持たない GET には一切不要**。＝ **164件の 503 は「何もしない関数」の順番待ちで出ていた**。

#### 是正 — 「S3 直行のビヘイビア」ではなく「Lambda@Edge だけ付けないビヘイビア」

🔴 **当初案（`/_next/static/*` を S3 オリジンへ直行させる）は成立しない。**
この distribution のオリジンは **`placeholder.sst.dev` の1本だけ**で、実際の振り分けは
viewer-request の CloudFront Function が KVS の `metadata` を読んで
`cf.updateRequestOrigin()` で毎回差し替えている（`metadata.s3.routes` に `/_next/static` がある）。
**指す先の S3 オリジンが distribution に存在しない。**

✅ 正しい形は **「同じオリジン・同じ CFF のまま、Lambda@Edge だけ付けないビヘイビアを足す」**。
CFF がそのまま S3 へ振り、OAC 署名も `setS3Origin()` が毎回付ける。

実装は `transform.cdn`（既に `webAclArn` を入れている場所）に `orderedCacheBehaviors` を1本足すだけ。
`CdnArgs.orderedCacheBehaviors` は SST の正式な引数で、ssr-site 側は
`transform.distribution` を使っていない＝**合成事故が起きない**ことを実装で確認した。

🔴🔴 **`functionAssociations` を default からコピーするのは必須。** 落とすと
**9（非本番の Basic 認証）と 12（apex→www の 308 ＋ HSTS）がこのビヘイビアだけ効かなくなる**
＝ dev の `/_next/static/*` が素通しになる。
📌 **「CloudFront Function は1ビヘイビアに1つ」の制約には触れない。** 制約は
*1ビヘイビア・1イベント型につき1つ* であって、**同じ関数を複数のビヘイビアに付けるのは可**。

#### ① クォータ引き上げは**やらない**（実数値で判断した）

| 指標（7日） | 実数 |
|---|---|
| CloudFront 総リクエスト | **2,540 ＝ 363/日** |
| うち Lambda@Edge を起動（Miss+503） | **1,253** |
| └ `/_next/static` | **1,032（82.4%）** |
| └ HTML 等 / `/api` / `/images` / `/_next/image` | 188 / 19 / 11 / 3 |
| `WebEdgeFn` 実行時間 | avg **4.8〜17ms**、max 80ms |
| 東京 server Lambda | 1,153 起動・**Throttle 0**・同時実行ピーク **10 が1回**（622バケット中） |

🔴 **「利用者が少ないから起きない」は誤り。** throttle は**量ではなく同時実行**で決まる。
363 req/日でも6回起きている。**訪問者1人でも 11 を超える。**
✅ **それでも①は不要。** 実測 164件のうち **159件（97%）が `/_next/static/` 配下**＝ ②だけで消える。
残る5件は `/backup/.git/config` 等を並列で舐めたスキャナで、ユーザー影響なし。
②後にクォータ 10 へ晒されるのは **207件/7日 ≒ 30件/日**（HTML/API）で、
ブラウザは HTML を1本ずつしか取らないのでバースト源にならない。
📌 **実ユーザーの経路はそもそもクォータの外**（server Lambda は東京1本＝ 1000）。
📌 10 のまま残せば**暴走時のコスト上限**としても働く。
⚠️ **コスト削減効果は無い**（Lambda@Edge は月 8,280 起動で **$0.01 未満**）。**これは信頼性の修正である。**

> 🔴🔴 **【2026-08-20 訂正】この判断の前提は不十分だった。**
> 「残る5件は `/backup/.git/config` 等を舐めたスキャナで、ユーザー影響なし」と数えたが、
> **08-13T17:50Z に同じ形のスキャナが 1,208 req/5分 を単一エッジ（SEA900）に撃ち、
> `LambdaLimitExceeded` の 503 が12件出た**（アクセスログで確認）。
> **#144 が経路から外したのは静的アセットだけで、未知パスは今も Lambda@Edge を通る。**
> 5件と1,208件を分けたのは設計ではなく**相手の規模**である。→ 教訓59。
> 対処の第一候補は**クォータ引き上げではなく WAF のサイト全体レートベースルール**
> （既存の3ルールはすべて `/admin` 限定）。
...
### 2026-08-09 — ✅ S-1・S-2 達成（soak の残りは S-3 のみ）

**`instagram-refresh` の初回スケジュール発火を実測した。** これで cron 4本すべてが
production で成功し（S-1 = 4/4）、Instagram トークンの更新経路も AWS 側で実証された（S-2）。

| 測ったこと | 結果 |
|---|---|
| CronRelay のログ | `03:30:10.411Z … instagram-refresh start` → `done 3003ms status=200` |
| `refreshedAt` | `2026-08-09T03:30:13.314Z`（スケジュール時刻と **13秒差**） |
| `expiresIn` | 5,184,000（60日）＝ **失効 2026-10-03 → `2026-10-08T03:30Z`** |
| **CloudTrail（03:00〜04:00Z の `Username=shun`）** | 🟢 **0件＝人の操作なし** |
| 直近の `ConsoleLogin` | 08-08T07:18Z ＝ **約20時間前** |

🔑 **教訓49 のとおり「値が進んだ」では止めず、「誰が動かしたか」まで測った。**
08-04（手動）のときは `ConsoleLogin` の**2分後**に invoke されていたが、今回はそれが無い。
🔴 **リクエストID の形は根拠に使っていない**（この実行も孤立形 `d16a77f4-…` で、
「孤立＝手動」は誤りだと判明した。訂正は教訓49 の節）。

**意味**: 依存 E・L（Instagram トークンは60日で恒久失効し、切れると手動再認証が要る）は
**AWS の週次で自走する**ことが実証された ＝ **15 で Vercel の月次 cron を消してよい。**

📌 soak の終了条件は **6つのうち5つ達成**。残るは **S-3（`2026-08-12T19:53Z` 到達）**だけ。

### 2026-08-13 — 🟢🟢 **S-3 達成＝14（soak）完了**。進捗 **19/21**、そして 15 ①（配線）

**soak の終了条件が6つとも埋まった。** 残っていた S-3（アラーム遷移ゼロが7日連続）を
到達予定 `2026-08-12T19:53Z` の**翌日に引き直した**（手順書 §0-1 が「当日に引き直す」と
指定しているため。事故が1回でもあれば起算が後退する）。

| 測ったこと | 結果 |
|---|---|
| `describe-alarm-history`（`2026-08-05T19:53:07Z` → `2026-08-13T17:55:35Z`・2リージョン） | 🟢 **0件**＝ **7日22時間 連続で遷移ゼロ** |
| **負の対照**（窓を `2026-08-01` まで広げる） | ap-northeast-1 **20件** / us-east-1 **12件** ＝ **クエリは効いている** |
| 現在の状態（12本・2リージョン） | **全て `OK`**（`INSUFFICIENT_DATA` も無し） |
| 直近の遷移 | `2026-08-05T19:53:06Z` の `siko-production-cloudfront-5xx` ALARM→OK ＝ **起算点そのもの** |
| 本番の応答 | `/` 200 ／ `/shop` 200 ／ apex 308 ／ `x-amz-cf-pop: KIX56-P4` |

🔑 **空の結果を信じる前に、同じクエリが非空を返す窓を1回踏む。** 教訓42・43 の合わせ技で、
ここは「アラームが鳴っていない」と「クエリが母集団を取れていない」が**同じ見た目**になる。

---

#### 15 ①（配線 PR）— 手順書に無い**2本目**の「宣言 ≠ 配線」がすぐ出た

`sst.config.ts` の `OPTIONAL_SECRET_NAMES` に決済3本（`STRIPE_SECRET_KEY` /
`STRIPE_WEBHOOK_SECRET` / `PAYMENTS_ENABLED`）を足した。**この時点で挙動は変わらない**
（既定値が空文字＝`isPaymentsEnabled()` は false のまま）。

🔴 **そこで、手順書の §3-2 がそのままでは実行できないことが分かった。**
手順書は「投入前に `npm run check:secret-file` を必ず通す」と書いているが、
`scripts/check-secret-file.mjs` の **`FORBIDDEN` に `/^STRIPE_/` と `/^PAYMENTS_ENABLED$/` が
入っていた**＝ **③で入れたいちょうど3本を、検査が必ず弾く**。
さらに ⑤（必須の充足）は 13 の一括投入（7本+11本を1ファイル）前提なので、
3本だけのファイルでは **`必須が欠けている` が7件出て exit 1** になる。

＝ **手順書どおりにやると必ず止まり、止まった人は検査を飛ばすほうへ動く。**
🔑 **`sst.config.ts` 側だけ配線しても「配線した」ことにならない。**
その秘密が通る**経路上のゲートも一緒に**開けないと、手順は完成しない。

対処（同じ PR に含めた）:

| 変更 | 理由 |
|---|---|
| 3本を `FORBIDDEN` → `OPTIONAL` へ移動 | 禁止の理由は「配線が無いので投入しても効かない」だった＝**理由のほうが先に消えた** |
| `FORMATS` に3本を追加（`sk_live_` / `whsec_` / `true`） | `sk_test_` と `pk_live_` の取り違えを**値を見ずに**止める（教訓36） |
| `--partial` を追加（⑤だけを省く） | ①重複・②空値・③禁止・④形式は**部分投入でも全部走る**。危険を作るのはそちら |
| 重複検査から `PAYMENTS_ENABLED` / `ADMIN_TOTP_REQUIRED` を除外 | 正しく設定すると**どちらも `true`**＝ 除外しないと**正しいファイルを誤検知で止める** |

**検証は7通りの対照で**（正3・負4）: 正しい3本 → `✓`／`--partial` 無し → **必須7件で exit 1**
（＝緩めたのが⑤だけである証拠）／`sk_test_`・`pk_live_`・prefix 無しの `whsec`・
`PAYMENTS_ENABLED=false` → **全部 exit 1**／本物の秘密の重複（`[SENSITIVE]` 相当）→ **今も止まる**／
`AWS_ACCESS_KEY_ID` → **今も禁止**。

📌 **`check:sst` にも負の対照を打った**（わざと型エラーを入れて exit 2・エラーが
`sst.config.ts` を名指しすることを確認 → 復元して exit 0）。worktree では
「別の木を検査して合格する」が実際に起きているため、**合格を信じる前に1回落とす**。

#### ついでに測った env のパリティ（Vercel が生きているうちにしかできない）

`src/` が読む `process.env.*` は40個。うち **`sst.config.ts` にまったく現れないものが5本**
あった（`SITE_URL` / `NEXTAUTH_URL` / `WEBAUTHN_RP_NAME` /
`NEXT_PUBLIC_GOOGLE_SITE_VERIFICATION` / `ADMIN_PASSWORD`）。
**Vercel 本番の33本と突合した結果、5本とも Vercel にも無かった** ＝ **移行で失ったものではない**。
消費側もすべて `|| '<正しい本番値>'` のフォールバック付き（`ADMIN_PASSWORD` はテスト専用）。
🟢 **切替による欠落ゼロ。** ただしこの突合は **Vercel を解約したらもうできない**。

### 2026-08-13（続き） — 🔴 **検証作業そのものが本番アラームを鳴らした** → 5xx アラームを率から件数へ

#165 をマージし本番デプロイ（`20:26:47Z` マージ → Lambda `LastModified 20:32:07Z`）。
**配線だけで挙動が変わっていないこと**を前後で実測した:

| | before | after |
|---|---|---|
| production web Lambda の env 総数 | 35 | **38** |
| 決済3本 | **無し** | **3本ともあり・すべて空文字** |
| `POST /api/checkout` | 503 | **503** |
| `/shop` の停止表示・`availability` | あり・`OutOfStock` | **あり・`OutOfStock`** |

**その直後に `siko-production-cloudfront-5xx` が発報した。**

```
ALARM 2026-08-13T20:31:32Z → OK 2026-08-13T20:51:32Z（20分）
理由: 5xxErrorRate 33.33% > 5.0
```

該当の5分バケットは **リクエスト3本・うち5xx が1本**。**その3本は上の表を取るための私の測定**で、
5xx は `POST /api/checkout` の **503 ＝ 決済停止中の設計どおりの応答**だった。
`AlarmRelay` は 2 invocations / Errors 0 ＝ **Slack に発報と復旧の2通が飛んでいる**。

🔴🔴 **これは誤報ではない。アラームの設計が、このサイトの規模に合っていない。**
平常トラフィックは **5分あたり数本**しかなく、**率**で見ると 5xx 1本で 20〜33% に跳ねる。
閾値 5% は事実上「**1本でも鳴る**」だった。しかも今は「決済停止中の 503」という
**設計どおりの応答が 5xx として数えられている**。

#### 対処 — **率をやめて件数で見る**（`5xxErrorRate` は率しか出ないので metric math で戻す）

```
件数 = (5xxErrorRate / 100) × Requests   … 閾値: 5分で 3件以上
```

**式は本番の実データで検証した**（`get-metric-data` に同じ式を流した＝デプロイ前に確かめられる）:

| 期間 | 5分あたり最大 | 新閾値(≥3) |
|---|---|---|
| 2026-08-05 の実障害（`LambdaLimitExceeded`） | **105件** | 🟢 鳴る |
| 2026-08-02 のスパイク | **109件** | 🟢 鳴る |
| **2026-08-13 の自作自演** | **1件** | 🟢 **鳴らない** |
| 2026-08-12 の平常時（37バケット）＝負の対照 | **0件** | 🟢 鳴らない |

⚠️ **引き換えに落ちる感度を明記する。** 5分で数本しか来ない時間帯に**全滅**しても、
件数が3に満たなければこの1本では鳴らない。そこは **②`web-server-errors`（Lambda の異常終了）と
Sentry** が受け持つ。**「率で拾えるが誤報だらけ」より「件数で拾えないが鳴ったら本物」**を採った。

#### S-3 の扱い（オーナー判断・2026-08-13）

規則は「**事故が1回でも起きれば起算がそこまで後退する**」。今回は自作自演で原因も完全に
説明できるが、**規則を曲げずに「アラームを直してから起算し直す」**を選択した。
🔑 理由: **直さずに7日待つと、待っている間に誰かが 503 を1本引いた時点でまた後退する**
＝ **測れない指標で待つのは、待っていることにならない。**

#### 🔴 **S-3 の新しい起算点 ＝ `2026-08-13T21:24:19Z`（到達 `2026-08-20T21:24Z`）**

**正本は「直したアラームが有効になった時刻」**＝ `AlarmConfigurationUpdatedTimestamp`
（production `2026-08-13T21:24:19.081Z` / dev `21:24:24.798Z`）。
🔑 **直前の遷移（OK 復帰 `20:51:32Z`）を起算にしない**のは、その間の指標が
**まだ壊れたアラームのもの**だから。**指標を直した瞬間より前の「遷移ゼロ」は、
遷移が無かったのか測れていなかったのか区別が付かない。**

デプロイ後の実測（`21:26Z`）: **`21:00Z` 以降の StateUpdate は2リージョンとも 0件**・
**12本すべて `OK`**。アラームは**置き換えではなく更新**された（`StateValue` が `OK` のまま・
`MetricName`/`Statistic` が `null` になり `Metrics[]` に3本の id が入った）
＝ **デプロイ自体は遷移を作っていない**。

📌 ①〜⑤（決済再開まで）は切り戻せるので、**S-3 の再達成を待たずに進めてよい**。
待つのは **⑥（Vercel 解約＝不可逆）の手前だけ**。

### 2026-08-08〜09 — Slack 通知の2つの穴（AWS 移行で生まれたもの）

C-4 のローテーションで `SLACK_WEBHOOK_URL` を再発行し、**それが正しいか確かめようとして**
連続して2つ見つかった。どちらも**確かめようとしなければ出てこなかった**。

**① 失敗を知る手段が無かった（PR #155）。**
`notifySlack` は `r.ok` を見ず `catch {}` で握り潰していた。
🔴 **webhook の値が間違っていると Slack は 403/404 を返すが、`fetch` は HTTP エラーで
reject しない**＝ **`catch` にすら入らず、成功と区別が付かない**。
再発行をミスしていれば **6か所の通知が黙って止まるだけ**だった。

**② 通知が飛ばない／遅れる（PR #156）。**
**AWS Lambda はハンドラが返ると実行環境をフリーズする**ので、`await` されていない
`fetch` はそこで止まり、**次の呼び出しが来るまで再開しない**。
呼び出し5か所のうち**3か所が投げっぱなし**だった（`void` / 裸）。

#### 🔑 効果は「第三者のタイムスタンプ」で前後比較できた

自分のログでは「エラーが出ていない」しか言えない（①の修正前は特に）。
**Slack 側の表示時刻**が、修正の前後をそのまま分けてくれた:

| 送信 | 送信時刻(JST) | Slack 表示 | |
|---|---|---|---|
| テスト1（`await` 前） | 20:01:11 | **20:05** | 🔴 **約4分遅れ** |
| テスト3（`await` 後） | 22:05:07 | **22:05** | ✅ 即時 |
| テスト4/5（`await` 後） | 14:10:31 | **14:10** | ✅ 即時 |

応答時間でも裏が取れた（同一エンドポイントで比較）:
**400（`notifySlack` の手前で return）0.125〜0.144s ⇔ 200（DB＋Slack）0.369〜0.461s**
＝ 差 **約0.23〜0.32秒**が「待つようになった分」。

🔑 **規則: 自分の観測手段が壊れている疑いがあるときは、系の外にある時計を使う。**
①の修正前は「ログが静か」＝「成功」とも「そもそも届いていない」とも読めた。
**Slack の受信時刻は自分たちのコードに依存しない**ので、そこだけが両者を分けられた。

📌 再発防止は **`notifySlack` だけを対象にした `no-restricted-syntax`**。
型情報の要る `no-floating-promises` は repo 全体に波及するので使わない
（soak 中に無関係な変更を増やさない）。**旧形に戻すと lint が落ちることを実測して確認した。**

### 2026-08-20 — 🔴 S-3 が再び破れた。ただし今度は**アクセスログで原因が2件とも割れた**

`siko-production-cloudfront-5xx` が **ALARM `02:02:39Z` → OK `02:09:39Z`**（SNS アクションは両方 success）。
到達予定日 `08-20T21:24Z` の **19時間前**だった。

| | |
|---|---|
| 破れた時刻 | `2026-08-20T02:02:39Z`（復帰 `02:09:39Z`） |
| 🔴 **新しい起算** | **`2026-08-20T02:09:39Z`** ／ 到達 `2026-08-27T02:09Z` |
| 他のアラーム | 前起算（`08-13T21:24:19Z`）以降 **StateUpdate 0件**（本番6本・ap-northeast-1）。遷移したのは CloudFront の1本だけ |
| 直近デプロイ | `2026-08-13T21:38Z` ＝ **この日は誰も何も触っていない**（08-13 と違い自作自演ではない） |
| 母数 | `01:55Z` の5分バケットで **1,874 req**・`5xxErrorRate 0.32%` ＝ **6件** |

#### 🔴🔴 調査の最初に「診断手段が無い」と誤って結論した（15分の遠回り）

`get-distribution-config --query DistributionConfig.Logging` が `Enabled: false` を返したので
「アクセスログが無い＝この6件の正体は永久に分からない」と書いた。**誤り。**

**R-4 は 2026-08-03 から稼働していた**（`siko-production-cloudfront-access-logs` /
us-east-1 / 30日保持 / 3.8MB 蓄積済み）。**standard logging v2 は `DistributionConfig.Logging` を
使わない**（実体は `LogDeliverySource`／`LogDeliveryDestination`／`LogDelivery`）＝
**v2 を使っている限り `Enabled:false` が正常**。`sst.config.ts` にその旨のコメントまで書いてある。

🔑 **1つのフィールドだけを見て「無い」と結論した＝負の対照が無い。**
しかも本ログの 08-03 に「R-4 が初めて『原因を言える』側で働いた」と自分で書いてある。
**記録を読み返せば防げた誤りだった。** → 教訓58

✅ **正しい引き方**（`filter-log-events` はハイフン入りフィールドを弾くので Logs Insights）:

```
fields @timestamp, `sc-status`, `cs-method`, `cs-uri-stem`, `x-edge-detailed-result-type`, `c-ip`
| filter `sc-status` >= 500 | sort @timestamp asc
```

#### 原因A（今日の6件）＝ **Instagram の署名付き画像URLが失効していた**

```
2026-08-20 01:57:18.471/.597/.609  500 GET /_next/image [OriginError]  c-ip 110.239.210.236
2026-08-20 01:59:27.001/.007/.062  500 GET /_next/image [OriginError]  c-ip 193.149.22.236
```

画像最適化 Lambda（`siko-coffee-production-WebImageOptimizerFunction-xxfdkkdu`）のログに原因がある:

```
⨯ upstream image response failed for https://scontent-nrt1-2.cdninstagram.com/... 403
Failed to optimize image vr [Error]: "url" parameter is valid but upstream response is invalid { statusCode: 403 }
```

📌 **Lambda の `Errors` は 0**（12 invocations / Throttles 0）。**アプリが 500 を「返した」のであって
Lambda が落ちたのではない**＝ Lambda メトリクスだけを見ていると永久に見つからない。

**メカニズム**（すべて実測）:

| 要素 | 実測値 |
|---|---|
| `/` の `cache-control` | `s-maxage=2, **stale-while-revalidate=2592000**`（**30日**） |
| 実際に返ってきた `age` | 592秒（＝すでに古い版を配っている・`x-cache: Hit`） |
| CloudFront のキャッシュポリシー | `MinTTL 0 / DefaultTTL 0 / MaxTTL 31536000` ＝ **オリジンの SWR をそのまま尊重する** |
| 失効した署名の `oe` | `6A851079` → **`2026-08-19T02:10:01Z`** ＝ リクエストの **23.8時間前**に失効 |
| いま配信中の HTML の署名 | `6A8C2E9C` → `2026-08-24T11:44:28Z`（**残り約4日**） |
| 署名の寿命 | **約5日8時間**（同じ画像の再署名が `08-19T03:44:28Z` → `08-24T11:44:28Z`＝秒まで一致） |
| 日次トラフィック | **99〜2,498 req/日** |

🔑 **`HTML の陳腐化寿命（最大30日）> 署名URLの寿命（約5日）` になった瞬間に必ず 500 が出る。**
CloudFront の SWR は「古い版を返してから裏で取り直す」ので、**アクセスの少ないエッジほど
何日も古い HTML を配る**。低トラフィックであることが緩和ではなく**悪化要因**になっている。

🔴 **これは監視ノイズではなくユーザー可視のバグ**（トップの Instagram セクションの画像が壊れる）。
08-17T01:10Z にも **1件**出ていた（閾値3件未満で鳴らなかっただけ）＝ **鳴らなかった日にも起きている。**

#### 原因B（08-13 の12件）＝ **スキャナ 1,208 req/5分 × Lambda@Edge のクォータ10**

同じログで 08-13T17:50Z も割れた。**S-3 を「達成した」と宣言した `17:55:35Z` の5分前**である。

```
/.env  /.ssh/id_ecdsa  /.claude.json  /.streamlit/secrets.toml  /gcp-service.json
/firebase-config.json  /configuration.php.bak  /@fs/root/rootkey.csv  /host.key ...
→ 503 [LambdaLimitExceeded] ×12
送信元: 単一IP 35.252.127.169 が **SEA900-P4（シアトル）経由で 1,208 req/5分**
```

実測した同時実行クォータ: **ap-northeast-1 だけ 1000 / us-east-1・us-west-2・ap-northeast-2・
eu-west-1 は 10**（08-07 の調査時と変わっていない）。

🔴🔴 **#144 は「静的アセットの経路」から Lambda@Edge を外しただけで、
未知パス（デフォルトビヘイビア）は今も Lambda@Edge を通る。**
08-07 の「① クォータ引き上げはやらない」の根拠は
**「503 の 97% は `/_next/static` 配下・残る5件はスキャナだがユーザー影響なし」**だったが、
その5件は**たまたま小さいスキャンだった**だけで、**スキャンの規模は攻撃側が決める**。
1,208 req/5分 を1つのエッジに撃たれれば 10 は即座に埋まる。→ 教訓59

📌 WAF の3ルールは**すべて `/admin` 限定**（`AdminLoginRateLimit` / `AdminGeoRestrictJp` /
`AdminUiBotChallenge`）＝ **サイト全体のレート制限は無い**。当日 WAF は 1,208 件を全部 Allowed。

#### 集計 — #144 以降の 5xx は**全部で3イベント・ノイズはゼロ**

`08-07T06:00Z`（#144 本番デプロイ）〜 `08-20T06:00Z` の全 5xx（アクセスログの実物）:

| 時刻 | 内容 | 件数 | 新アラーム(≥3) |
|---|---|---|---|
| 08-13T17:50Z | スキャナ → `LambdaLimitExceeded` 503 | **12** | 🔔 鳴る（当時は率ベースで鳴らなかった） |
| 08-13T20:25/20:30Z | 自作自演の `POST /api/checkout` 503 | 1+1 | 🔕 鳴らない（#166 の狙いどおり） |
| 08-17T01:10Z | `/_next/image` 500（Instagram 失効） | **1** | 🔕 鳴らない |
| 08-20T01:55Z | `/_next/image` 500（Instagram 失効） | **6** | 🔔 鳴った |

✅ **#166 の修正は設計どおり効いている**（1件では鳴らず、6件では鳴る）。
🔑 **「原因不明の 5xx」は1件も無かった。全部に名前が付いた。**

#### 🔴 S-3 は「待つ」では埋まらない — 確率で評価する

新アラーム基準（5分3件以上）を満たすイベントは **13.0日で2回 ＝ 平均 6.5日に1回**。

| | |
|---|---|
| 7日連続クリアできる確率 | `exp(-7/6.5)` ≈ **34%** |
| 期待所要 | 7 ÷ 0.34 ≈ **約21日**（外すたびに振り出し） |

🔑 **原因が2つとも既知になった今、正しい順序は「直す → 引き直す」**であって「待つ」ではない。
**達成条件は、達成確率が計算できるなら計算してから待つかどうかを決める。** → 教訓60

📌 **この日は記録のみで、修正はまだ入れていない**（次の作業として起票）。

### 2026-08-20（同日・続き） — ✅✅ C-6 / C-7 / T5 を本番へ。**S-3 の起算は `07:43:45Z`**

朝に S-3 が3回目の破綻をしてから、**同日中に発生源を両方潰して本番稼働まで到達した**。
「λ を下げずに起算し直すのはループ」（教訓60）に対する具体的な答えがこの1日である。

CI 全通過 → squash マージ **`411a62d`（#168）** → **SST Deploy (dev / production) とも success**。

#### 入れたもの

| | 内容 | 効く相手 |
|---|---|---|
| **C-6** | `/` を**6時間ごと**に CloudFront から無効化する cron（`CronRefreshHome` / `cron(42 0/6 * * ? *)`） | SWR 30日 ≫ 署名寿命 約5日 の逆転を、**陳腐化の上限6時間**で解消 |
| **C-7** | WAF に `SiteWideRateLimit`（priority 3 / Block / **600 req / 300秒 / IP**） | 1,208 req/5分 のスキャナだけを跨ぐ位置 |
| **T5** | ALARM 通知に Logs Insights クエリとロググループの探し方を同梱 | 教訓58（引かれない診断は腐る）の**系側**の対処 |

#### 🔑 専用 Lambda を作らず既存の `CronRelay` に相乗りさせた

理由は3つあるが、**いちばん重いのは1つ目**である。

1. 🔴 **新しい Lambda は新しいアラームを要し、新規アラームは必ず
   `INSUFFICIENT_DATA → OK` の遷移を1回作る**＝ **S-3 の観測面をわざわざ増やす**ことになる。
   「S-3 を回復させるための変更が S-3 を壊す」という 08-13 と同じ形を、設計の段階で避けた。
2. `retries: 2` とログ形式（`[cron] relay …`）と `cron-relay-errors` アラームをそのまま継承できる。
3. 「4本のスケジュールが1つの中継関数を共有する」という**この関数の元々の設計思想**そのままで、
   増えるのは event の1フィールドだけ。

📌 **循環参照を避けるためでもある。** distribution の ID は `sst.aws.Nextjs` が作り終えて
初めて確定するので、**Web の Lambda 自身の環境変数には入れられない**（自分自身を参照する）。
中継は web の後に作られるので渡せる。

#### 実測（デプロイ後・すべて取り直した）

| # | 見たもの | 結果 |
|---|---|---|
| ① | 本番 Web Lambda の `LastModified` | **`2026-08-20T07:43:45Z`** ✅ |
| ② | `CronRefreshHomeSchedule` | production / dev とも **ENABLED** ✅ |
| ③ | `CronRelay` の `CLOUDFRONT_DISTRIBUTION_ID` | **`E3FC7N27IY6A73`**（ドメイン側の `d38zi1bm4zf9e3` ではない）✅ |
| ④ | WAF | `SiteWideRateLimit` / Block / limit **600** / window **300** / priority 3 ✅ |
| ⑤ | **負の対照** | `/`・`/shop`・`/legal/privacy` とも **200**（正常な閲覧はブロックされない）✅ |
| ⑥ | **無効化の経路** | 下記 ✅ |
| ⑦ | 本番アラーム7本 | **全部 OK。デプロイによる遷移はゼロ**（`cloudfront-5xx` の最終更新は今朝の復帰 `02:09:39Z` のまま）✅ |

**⑥ は「作れた」で止めずに撃った（教訓53）。** 次のスケジュール発火は 12:42 UTC だったので、
待たずに `aws lambda invoke` で1回だけ実行し、**権限とコードの両方が本番で通ること**を確かめた。

```
応答            {"invalidationId":"ICN698DLYK3FSRX364JJWBLAE3"}（StatusCode 200 / FunctionError なし）
CloudFront 側   Paths: ["/"] のみ ／ CallerReference: cron-invalidate-29786866（設計どおり分単位）
中継のログ      [cron] relay invalidate start distribution=E3FC7N27IY6A73 paths=/
                [cron] relay invalidate done 1542ms id=ICN698DLYK3FSRX364JJWBLAE3 status=InProgress
```

📌 デプロイ後の `/` は `cache-control: s-maxage=3506, stale-while-revalidate=2592000` ／ `age: 1`
＝ **今日 500 を出した「STALE な `s-maxage=2`」ではない**状態に戻っている。

#### 🔴 S-3 の新しい起算点 ＝ `2026-08-20T07:43:45Z`（到達 `2026-08-27T07:43Z`）

**正本はこの文書に書かれた値そのもの**＝ `2026-08-20T07:43:45Z`。
取得元は当時の本番 Web Lambda の `LastModified`（#168 のデプロイ）である。

🔴 **後から `get-function-configuration --query LastModified` を叩いても、この値は返らない。**
docs だけの PR でも main への push は本番デプロイを走らせる（`ci.yml` の `matrix.stage`）ので、
**`LastModified` はその後のマージのたびに前へ進む**。**起算点は「いま測る値」ではなく
「あのとき測った値」**なので、ここに書いた文字列を正本として扱うこと。
（📌 再デプロイ自体はアラーム遷移を作らない＝ S-3 は壊れない。08-13 に実測済みで、
今回の⑦でも確認した。）

🔑 **今朝の OK 復帰 `02:09:39Z` から数えることもできた**（遷移はその後ゼロのままで、
アラーム自体は壊れていないので測定は有効だった）。**それでもデプロイ時刻を採る。**
その5.5時間は **C-6・C-7 が生きていた設定での「静かさ」**であって、
いま数えたい「直した後の7日」ではない。08-13 に採った規則
（**指標を直した瞬間より前の「遷移ゼロ」は信用しない**）と同じ形で、対象が
「指標」から「発生源」に変わっただけである。差は5.5時間で、規則を曲げてまで取りに行く価値はない。

⚠️ **今回は前2回と条件が違う。** 08-05 と 08-13 の再起算は **λ を下げないままの引き直し**
だったが、今回は**発生源を両方潰してからの引き直し**である。#144 以降の発報相当2件は
どちらも C-6 / C-7 に帰着したので、**既知の発生源は残っていない**。

#### まだ確かめていないこと（正直に残す）

1. **12:42 UTC の初回スケジュール発火**（人の操作ゼロで動くこと）。⑥ は手動実行なので、
   **「スケジュールが引く」ことの証明にはなっていない**（08-09 の S-2 と同じ区別）。
2. `/` の `age` が **21,600秒（6時間）を超えないこと**。
3. WAF の `BlockedRequests`（`siko-production-site-wide-rate-limit`）が
   **実ユーザー経路で立たないこと**。⑤ は打った瞬間の1点でしかない。

---

### 2026-08-21 — ✅ 15 の ②③④ を通した（**決済再開・買える状態**）／残りは⑤の実購入だけ

S-3（到達 `08-27T07:43Z`）を待つ7日を空費しないために、**S-3 と独立に進められる ②〜⑤** を進めた。
①（配線 PR・#165）は 08-13 に済んでいたので、この日は **② Stripe 側の確認 → ③ 投入 → ④ デプロイ → ⑤ の 1〜3** まで。

#### ② Stripe 側（オーナー作業）— **3つとも「変更不要」で終わった**

| 前提 | 実測 |
|---|---|
| Webhook エンドポイント URL | ✅ 既に `https://www.sikocoffee.com/api/webhooks/stripe`（Vercel 時代のままではなかった） |
| エンドポイントの状態 | ✅ `Active`（連続失敗による自動無効化を受けていない） |
| 送信イベント | ✅ `checkout.session.completed` / `checkout.session.expired` / `charge.refunded` の3種ちょうど |
| `sk_live_...Nk7B` | ✅ 存在・**最終使用の記載なし** ＝ Phase 0 以降どこにも投入されていない証拠 |
| 署名シークレット | ✅ 取得してパスワードマネージャへ |
| Home のバナー | ✅ 警告なし（アカウント制限・入金口座の不備なし） |

🔑 **エンドポイントを作り直さずに済んだので、runbook 0-2 の「1〜3 はこの順に」という順序制約は発生しなかった。**
（作り直すと `whsec_` が変わる＝先に URL を確定してから取る、という制約だった）

#### 経路の実測 — **Stripe を呼ばずに「Stripe から叩けるか」を確かめた**

```
POST https://www.sikocoffee.com/api/webhooks/stripe  → 400 {"error":"Missing signature"}  リダイレクト 0 回
POST https://sikocoffee.com/api/webhooks/stripe      → 308 → www          ← 負の対照
```

400 は **route.ts の最初のガードが実行された証拠**＝ CloudFront → Lambda まで到達し、
WAF の `SiteWideRateLimit`（#168 で入れたばかり）にも当たっていない。
負の対照が意味を持つのは、**Stripe が webhook 配信でリダイレクトを追わない**ため。
apex 宛のままなら**全配信が失敗し、アプリ側にはログすら残らなかった**。

#### ③ 投入 — 検査は通ったが、その前に**デプロイ元の基点**で1件見つかった

`npm run check:secret-file -- --partial` は3本とも通過（値は出さず `len` と `sha` だけ）。
`len=107 / 38 / 4` ＝ `sk_live_` / `whsec_`＋32桁 / `true` と整合。

🔴 **`git fetch` したら、このワークツリーが main より1コミット遅れていた。**
HEAD は `8d901ef`（**#169**）なのに `origin/main` は `a7c7d48`（**#164**）。
＝ **PR 番号はマージ順ではない**（#164 のほうが後にマージされた）。
`git log` の見た目や番号の大小で「自分が先端」と判断すると、
**④で古いコードを本番へ送る**ことになる。`--ff-only` で早送りしてから投入した
（差分は docs と eslint 設定のみで、アプリのコードには影響なし）。

#### ④ デプロイ — **Claude では実行できず、オーナーが手で打った**

`npm run sst:deploy -- --stage production` が **auto mode の分類器にブロックされた**。
📌 このとき ③ と ④ の間で止まったが、**壊れた状態にはならなかった**。
値は SST の state にあるだけで Lambda の env は空のまま＝ `isPaymentsEnabled()` は false。
**フェイルクローズ設計のおかげで「途中で止まっても停止側に倒れる」**（→ 教訓61）。

#### ⑤ の 1〜3 — **変更前に取ったベースラインと同じ手段で全項目が反転**

| 見たもの | 変更前 | 変更後 |
|---|---|---|
| Lambda env の長さ（値は非表示） | `0 / 0 / 0` | **`107 / 38 / 4`**（投入ファイルの検査値と一致） |
| `/shop`・`/shop/catalog`・`/shop/product/brazil` | 停止告知あり | **告知なし**・全て 200 |
| 構造化データ `availability` | `OutOfStock` | **`InStock`** |
| `POST /api/checkout` | 503 | **400** |

🔑 **最後の1行が今回の発明**。`isPaymentsEnabled()` が**最初の関門**なので、
**わざと不正なボディ（JSON）を送ると、閉じていれば 503・開いていれば 400** になる。
＝ **Stripe セッションを1つも作らず・注文も残さず・5xx も出さずに、ゲートの開閉だけを測れる**（→ 教訓61）。

**S-3 への影響なし**: 起算 `2026-08-20T07:43:45Z` 以降のアラーム遷移は2リージョンとも 0 件
（窓を 08-01 まで広げると 20 件出る＝**負の対照つき**）。この日に本番へ出した応答は 400 と 200 だけ。

#### 残り（⑤ の 4〜6）— **オーナー作業。ここを飛ばして「再開できた」と書かない**

1〜3 は「入口が開いた」までしか言わない。`STRIPE_WEBHOOK_SECRET` の誤りは
**決済が成功したあとにしか顕在化しない**（教訓37 と同型）。

| # | やること | 何が閉じるか |
|---|---|---|
| 4 | 少額の商品を実際に1件買う | Stripe セッション作成 → 決済 → **webhook で注文が確定**まで通る |
| 5 | 注文確認メールの**照会リンク**を開く | 🔑 `ORDER_TOKEN_SECRET` のローテーション検証（C-4 の残り2本のうち1本） |
| 6 | 注文メールが SES で届く | R-5 の設定セットのイベントにも出る |
| 任意 | そのあと Stripe から**返金** | 3つ目のイベント `charge.refunded` の疎通（手数料は戻らない＝検証コスト） |
...
🔴🔴 **追記（2026-08-07）: この教訓自体が半分しか実行されていなかった。**
上で「`tsconfig.sst.json` ＋ `npm run check:sst` を用意して塞いだ」と書いたが、
**`check:sst` は `ci.yml` のどのジョブにも入っていなかった**
（`Lint & Type Check` が実行していたのは `check:cron` / `check:sst-config` /
`lint` / `tsc --noEmit` / `test` の5本だけ）。
＝ **道具は作ったが「壊す前に止まる場所」に置いていなかった**——この教訓が
名指しで禁じている形そのものを、この教訓を書いた本人が踏んでいた。
負の対照（`action: 'lambda:InvokeFunctionUrl'` を `action: 12345` に）で実測すると、
`tsc --noEmit` ・ `eslint` ・ `check:sst-config` は **3つとも exit 0**、
`check:sst` だけが **exit 2 / TS2322** を返す。
🔴 CI に足すには **`npx sst install` を前に置く**必要がある。型を供給する
`.sst/platform/config.d.ts` は gitignore 対象＝チェックアウト直後には無く、
省くと「常に赤い」ステップになって**本物の型エラーと見分けが付かなくなる**
（providers を入れるだけなので **AWS 資格情報は要らない**＝ PR でも走る）。
🔑 **「塞いだ」と書くときは、その道具が実行される経路に繋がっているかまで確かめる。**
**道具の存在は、実行の証拠ではない。** 教訓23（`| tail` で失敗が exit 0 に化ける）と
同じ穴——**「検査を書いたこと」と「検査が走ること」は別の事実**。
...
### 45. **症状の名前は原因ではない — 上限に当たったなら「その上限は何か」まで降りる**

`x-edge-result-type: LambdaLimitExceeded` は、読めば分かる立派な名前が付いている。
だが**それは症状の名前であって原因ではない**。ここで止まると出てくる対策は
「Lambda@Edge を軽くする」「リトライする」といった見当違いのものになる。
実際の原因は **アカウントの同時実行クォータが `ap-northeast-1` だけ 1000・他は全部 10** で、
そこまで降りて初めて **「①クォータを上げる」と「②関数を呼ばない」の2択**が見える。

🔑 **上限に当たったら、必ず上限そのものの値を引く。** 引いてみたら
**リージョンごとに 100 倍違った**。「AWS の既定は 1000」という一般知識のまま
「1000 に当たるほどのトラフィックは無い」と考えると、**永久に見つからない**。
（教訓43 と同じ形＝**リージョン差は知っていても、確認は片方で済ませてしまう**。）

🔴 **さらに、症状の名前は「誰が悪いか」まで誤らせる。** `LambdaLimitExceeded` は
Lambda@Edge が重い・遅いという印象を与えるが、実物は **685 バイトで GET では即 return** し、
実行時間は **avg 4.8ms** だった。**遅いから溢れたのではなく、同時に来たから溢れた。**
🔑 **`Errors` が 0 であることが決め手になった**＝「落ちた」と「絞られた」を区別する負の対照。
関数の中身を読むまで「何もしない関数の順番待ちで 164 件の 503 が出ていた」とは分からない。

🔑 **量の指標で見ると、この種の障害は永久に説明が付かない。** 実測は **363 req/日**で、
「利用者が少ないから起きないはず」と読める。だが throttle は**同時実行**で決まり、
ブラウザは1ページで静的アセットを数十本**同時に**取りに行く。
＝ **訪問者1人でも上限を超える。** 平均値で安全を語ると、バーストで壊れるものを見落とす。
### 46. **暗号化は「保管した場所」ではなく「値が通る経路すべて」で成立する**

`sst secret` に入れた17本は `secret/<app>/<stage>.json` では**暗号化されていた**。
にもかかわらず、**同じ値が `app/<app>/<stage>.json` に平文で入っていた**（2026-08-07 実測）。
理由は `sst.aws.Nextjs` がビルダー（`command:local:Command`）に **`process.env` を丸ごと渡し、
それが Pulumi の state 入力としてそのまま保存される**から。
`SST_SECRET_*` は SST 自身が環境変数に展開するので、**暗号化した値が同じプロセスの `process.env`
に戻ってきて、そこから平文で state に落ちていた**。

🔴 **「シークレットは暗号化して保管している」は、保管先を1つ確かめただけでは言えない。**
今回は暗号化した側と平文の側が**同じバケットの隣のプレフィックス**にあった。

同じ形で3つ巻き込まれていた:

- `CRON_SECRET` は **08-02 に衛生目的でローテーションしたばかり**で、
  新しい値が **08-03 のデプロイで平文に載った**
  ＝ 🔑 **ローテーションは「その先で平文に落ちる経路」を塞がないと成果が消える。**
- GitHub OIDC を採り「静的キーをリポジトリに置かない」を達成していたのに、
  **OIDC が発行した一時資格情報のほうが state に845セット残っていた**
  ＝ 🔑 **資格情報を短命にすることと、短命な資格情報を長期保管しないことは別の対策。**
- 平文は state 本体だけでなく **`snapshot/` と `eventlog/` にも同じだけ入っていた**
  ＝ 📌 **保管先は1つではない。** 容量の棚卸し（2026-08-04）はこの3つを
  「大きいファイル」としては数えていたが、**中身は見ていなかった**。

🔑 **規則: シークレットの棚卸しは「どこに入れたか」ではなく
「どのプロセスの環境変数に載るか」から辿る。**
環境変数に載った時点で、そのプロセスが書くあらゆる成果物（state・ログ・イベント・
`sst diff` の標準出力）が保管先の候補になる。
...
### 49. **「値が進んでいた」は「意図した仕組みが動いた」と同じではない**

soak の S-2（Instagram トークンが AWS 経由で更新されるか）を確認しようと `refreshedAt` を引いたら、
記録の `2026-08-01T00:21:11Z` から **`2026-08-04T17:41:16Z` へ進んでいた**。
**ここで「AWS の週次が動いた＝達成」と書けば、それらしく辻褄が合ってしまう。**

合わなかったのは**曜日**だった。AWS は `cron(30 3 ? * SUN *)`、Vercel は毎月1日。
**08-04 は火曜**で、どちらにも当たらない。追うと CloudTrail に
`ConsoleLogin`(17:39:16Z) → invoke(17:41:14Z) → `FilterLogEvents`(17:41:48Z) が並んでいた
＝ **人がコンソールから手動でテスト実行していた**。
（Lambda の invoke はデータイベントなので CloudTrail 本体には出ないが、**前後の管理イベントで挟める**。）

🔴🔴 **訂正（2026-08-09）: 当時もう1つ挙げた「リクエストIDの形」は根拠にならない。**
「定期実行の `release-reservations` は連番風（`106a72XX-…`）なのに、この1本だけ孤立した
`6bd9f0ea-…` だった」を傍証として書いたが、**08-09 の“スケジュール”実行も同じ孤立形
（`d16a77f4-…`）だった**。連番風に見えていたのは **`release-reservations` という
そのスケジュール固有の並び**にすぎず、**「孤立＝手動」ではない。**

🔑 **規則: 結論が正しくても、根拠は1本ずつ検証する。**
このときの結論（08-04 は手動）は CloudTrail で正しく取れていた。
だからこそ**間違った傍証が一緒に残り**、「次も同じ見方で判定できる」という誤った道具になった。
**当たった推論の中に混ざった外れた根拠は、外れた結論より見つけにくい**
＝ 結論が合っている限り誰も検算しないため。
📌 **今回の判定は CloudTrail 1本に絞った**（03:00〜04:00Z に `Username=shun` のイベントが
ゼロ・直近の `ConsoleLogin` は約20時間前）。**「複数の傍証がある」は強さではなく、
弱い根拠を混ぜる入口になりうる。**

🔑 **規則: 「変化したこと」を確認したら、次に「誰が変化させたか」を確認する。**
値の変化は本物でも、**変化させた主体が違えば結論は逆になる**。
今回それを飛ばしていたら、**15（Vercel 解約）で月次を消した後に
「AWS の週次は一度も発火していなかった」**という最悪の形で表に出ていた。

📌 副産物として**失効期限が 09-30 → 10-03 に伸びていた**ことも同時に分かった。
＝ **予定を書いた場所（`refreshedAt` 起点の計算）も、実測しないと古くなる。**

### 50. **文字列が「出るか出ないか」で判定すると、名前と値を区別できない**

C-4 で state の平文を削除した後、残ったオブジェクトを走査したら
`eventlog/` に **`SST_SECRET_` が17回**出た。`SST_SECRET_` は**変数名にも現れる**ので、
出現の有無だけでは「値が入っている」とは決まらない。

🔑 **規則: 「秘密が入っているか」を確かめるときは、
秘密の“ラベル”ではなく“中身の形”を探す。**
ラベルで探すと**偽陽性**（名前を見て残存と誤認）が出る。
📌 教訓46「暗号化は値が通る経路すべてで成立する」の検証側の対。
**塞いだことを確かめる grep も、塞ぐ対象と同じ精度で設計しないといけない。**

🔴 **この規則は正しいが、当時それを適用して出した結論は間違っていた。→ 教訓52。**

### 51. **境界は決め打ちせず実測する／1点の実測を代表値として記録しない**

同じ C-4 で2つ出た。

**① 境界の決め打ちが外れていた。** 「#146 のデプロイ完了時刻 `08-07T06:25:24Z` より古い版が汚い」
と置いて削除対象を組んだが、**その2秒前 `06:25:22` の版は既にクリーンだった**。
279バージョンを等間隔にサンプルして、**汚い最後 `06:19:20` / きれいな最初 `06:25:18`** を
実測し直した。**イベントの記録時刻と、その効果が state に現れる時刻は一致しない。**

**② 「変更前は34」は1サンプルの値だった。** 初回調査はある1版の `SST_SECRET_` 出現数 34 を
「変更前の値」として記録していたが、母集団を見たら**版によって 34〜68**で揺れていた
（チェックポイントに含まれるリソース数で変わる）。

🔑 **規則: 1点で測った数字は「その1点の値」として書く。**
代表値のつもりで書くと、後で母集団を見たときに**「合わない＝どちらかが間違い」**という
無駄な調査が始まる。**削除や移行の対象を決める閾値なら、なおさら範囲で測る。**

### 53. **「本番でしか出ない条件」は、面倒でも人が1回やる — S-5 はそれで1本捕まえた**

soak の終了条件のうち **S-5（本番でアイコンを1回上げる）だけは自動化していない**ので、
ずっと後回しになっていた。2026-08-08 に実施したら **1回目で失敗した。**

原因は CSP で、`connect-src` にアップロード先の S3 が無く、
**ブラウザが presigned URL への PUT を送信する前に止めていた**。
4（Vercel Blob → S3）でストレージは移したのに CSP を直しておらず、
`img-src` には `*.cloudfront.net` を足してあったので **表示（取得側）は通っていた**。

🔴 **だからテストでも目視でも出なかった。** UI は「アップロードに失敗しました」だけ、
**サーバ側のログには何も出ない**（ブラウザが送信前に止めるので、リクエストが届いていない）。

🔑 **規則: 経路を丸ごと差し替えたら、「取得」と「送信」を別々に確かめる。**
片方だけ直すと**動く経路と動かない経路が混ざり**、
「一部は動いているのだから設定は合っているはず」という誤った安心が生まれる。

🔑 **もう一つ: 15（切り戻せなくなる作業）の前に置いたからこそ意味があった。**
これを 15 の後に見つけていたら、**Vercel へ戻せないまま直す**ことになっていた。
＝ **終了条件は「確認のための儀式」ではなく「取り返しがつくうちに壊す」ための仕掛け。**

📌 修正後は4段すべてを実測した（公開バケットの実体／Rekognition がちょうど1回／
`pending/` が0件＝後片付け／DynamoDB の更新）。**ブラウザで見えたことは証拠にしない**
＝ CDN のキャッシュを見ている可能性があるため。

### 54. **`aws login` が成功していても、SST からは資格情報が見えないことがある**

`aws login` 直後なのに `sst secret set` が
`AWS credentials are not configured` ＋ `no EC2 IMDS role found` で落ちた。
**資格情報が切れていたのではなく、SST から見えていなかった。**

`~/.aws/config` が **`login_session`**（新しい `aws login` の方式・キャッシュは `~/.aws/login/`）
を使っており、**`~/.aws/credentials` は存在しない**。
`aws` CLI はこの方式を解釈するが、**SST は Go のバイナリで `login_session` を理解しない**
→ 資格情報が見つからず **IMDS へフォールバックしてタイムアウト**する。

🔑 **対処は環境変数への展開**（`scripts/deploy.sh` が既にやっていること）:
...
＝ **`npm run sst:deploy` は動くのに `npx sst …` は動かない**理由がこれ。
📌 実測で対照を取った: 展開なし → `credentials are not configured` ／
展開あり → `Secret "…" does not exist`（＝**認証を通り state を読めている**）。

🔑 **`sst secret set <name>` は値を省略でき、標準入力から読める**
（`printf '%s' "$V" | npx sst secret set NAME --stage production`）
＝ **シェル履歴に秘密を残さずに投入できる。**

### 52. 🔴 **対照は保存形式ごとに作り直す — 「中身の形を探す」の“形”は1つではない**

**教訓50 を立てた同じ作業で、その教訓を適用して出した結論が間違っていた。**
C-4 を「state 内の平文はゼロ」と結論づけ、**PR を出してマージまでした**。
数時間後、マージによるデプロイが書いた state を見て誤りが分かった。

**何を間違えたか。** 教訓50 に従って「ラベルではなく中身の形」を探すことにし、
値だけが持つ特徴として **`"ASIA`**（一時アクセスキーの先頭・**引用符付き**）を対照にした。
`app/` はこれで正しく判定できた（値が実在した版は 4〜8件・secret 化後は 0件）。

🔴 **しかし `eventlog/` は JSON ではなく `NAME,,,VALUE` のカンマ区切りで、値に引用符が付かない。**

```
SS_KEY_ID,,,ASIA****        ← 実際の格納形式
```

`"ASIA` は **0件**を返し、それを「値は入っていない」と読んだ。**値は最初からそこにあった。**
正しい検出は **`SST_SECRET_[A-Z0-9_]*,,,`**（17件一致）と
**`IQoJb3JpZ2lu`**（STS セッショントークンの base64 先頭・2件一致）だった。

🔑 **規則: 対照（negative control）は、探す先の保存形式ごとに作り直す。**
**1つの形式で正しく働いた対照を、別の形式にそのまま持ち込むと、
「測った」という手応えだけが残って中身は偽陰性になる。**
...
「`production.json` で 34 → 0」を確認して**塞げたと結論した**が、
確認したのは**対策を入れた場所**であって、**秘密が通る経路の全体ではなかった**
＝ **教訓46 と全く同じ誤りを、その教訓を書いた本人が検証側で繰り返した。**

📌 **手戻りの実際**: 削除した1,102件は無駄ではなかった（`app/` は実際にきれいになった）。
壊れたのは**「終わった」という判定**のほうで、その判定を**ドキュメントとメモリと PR 本文に
書いて main に入れた**ため、訂正のコストが作業そのものより大きくなった。
🔑 **「完了」と書く前に、"どの範囲を測ったか" を1行で言えるか自問する。**
言えないなら、それは完了ではなく**測った範囲の報告**である。

---

### 55. 🔴 **禁止も「なぜ要るか」を失うと腐る — そのとき禁止は、正しい作業を止める側に回る**

このプロジェクトは「**積み残しは理由を持たないと腐る**」を3回踏んでいる（5-2・5-3・15 の
「販売再開の手順」）。どれも **やること だけが残り、理由 が先に消えていた**。
15 ①で出たのは、その**裏返し**だった。

`check-secret-file.mjs` の `FORBIDDEN` に `/^STRIPE_/` と `/^PAYMENTS_ENABLED$/` があった。
入れた理由は正しい:「決済停止中で `sst.config.ts` に配線が無い＝**投入しても効かないので、
入れようとした時点で何かを取り違えている**」。
ところが 15 ①で配線した瞬間、**理由だけが消えて禁止が残った**。
残った禁止は「入れてはいけない」ではなく **「正しい手順が必ず失敗する」** を意味する。

🔑 **規則: ゲート（禁止・必須・検査）は、それを置いた条件と一緒に書く。**
条件が変わったときに**一緒に動かせる形**にしておく。条件を書かない禁止は、
**後から見ると「昔から駄目なんだろう」としか読めない**ので、誰も外せなくなる。

🔴 **もう一段悪いのは、止まった人の動き方。** 手順書が「必ず検査を通せ」と書いてあり、
その検査が**通しようがない**とき、人は検査を外す方向に動く（`--force` を探す・
目視で済ませる）。**壊れたゲートは、無いゲートより悪い。**

📌 **見つかったのは、手順書を「読んだ」からではなく「実行可能かを確かめた」から。**
§3-2 のコマンドを**ダミーの値で1回流した**だけで出た。
🔑 **手順書は、書いた直後に一度“空撃ち”する。** 値が無くても、形式検査と
禁止リストは本番と同じ経路を通る。

### 56. **「配線した」は、秘密が通る経路の**全部**を開けて初めて言える**

「宣言 ≠ 配線」は #125・#128・#160 で3回踏んでいて、15 の手順書はそれを見越して
「①配線 PR を先にやる」と**先回りして**書いてあった。それでも1本漏れた。
漏れたのは `sst.config.ts` ではなく、**その手前の検査スクリプト**である。

＝ **「配線」を `sst.config.ts` だけの話だと思っていた。** 実際に秘密が通る経路は

```
手元のファイル → check-secret-file.mjs（ゲート①）→ sst secret load → SSM
   → sst.config.ts の environment（ゲート②）→ Lambda の process.env → 消費側のコード
```

で、**ゲートは2つある**。②だけ開けても①で止まる。
🔑 **経路を端から端まで書き出してから、各段が開いているかを1つずつ見る。**
「いちばん忘れられがちな段」は毎回違うので、**前回忘れた段を覚える**のではなく
**毎回書き出す**ほうが速い。

### 57. 🔴 **率で見る監視は、母数が小さいと「1件でも鳴る」に化ける — しかも自分の検証が鳴らす**

`siko-production-cloudfront-5xx` は `5xxErrorRate > 5%` だった。**閾値としては妥当に見える**。
だがこのサイトの平常トラフィックは **5分あたり数本**で、**5xx が1本出ると 20〜33%** になる。
＝ **「5%」は「1本」と同義**であり、**閾値を書いた時点でそう決めたつもりは無かった**。

2026-08-13 に実際に鳴らしたのは **検証作業そのもの**だった。
デプロイ後に「決済がまだ止まっていること」を確かめるために `POST /api/checkout` を1回叩き、
**その設計どおりの 503 が、3本しかないリクエストの1本**だったため 33.3% になった。

🔑 **規則: 率のアラームには「母数がいくつのとき、何件で閾値に届くか」を必ず併記する。**
書けないなら、その指標は**率ではなく件数**で見る。
率は「母数が十分あるとき」にしか意味を持たない量で、**深夜と昼で意味が変わる指標を
1つの閾値で見張ることはできない**。

🔴 **もう一つ: 「設計どおりのエラー応答」は監視から見ると本物のエラーと区別が付かない。**
決済停止中の 503 は**仕様**だが、CloudFront にとってはただの 5xx である。
**意図的に返しているエラーがあるなら、それが監視に何を見せているかを1回数える。**

📌 **観測が対象を変えた例でもある。** S-3（アラーム遷移ゼロ）を測るための作業が
S-3 を破った。**測定のコストには「対象への影響」も含まれる**ので、
本番に対する測定は**何を発生させるか**まで見てから打つ。
（今回でいえば、`/api/checkout` の 503 は**アプリのログでも確かめられた**＝
本番にリクエストを打つ必要が無かった。）

### 58. 🔴 **入れた診断は、引かれるまで「無い」のと同じ／「無効に見えるフィールド」は版が違うだけかもしれない**

2026-08-20、6件の 5xx の原因を調べて **「アクセスログが無いので永久に分からない」と結論した。
実際には R-4 が17日前から動いていて、引いたら3分で原因が割れた。**

2つの失敗が重なっている。

1. **版の取り違え。** standard logging は **legacy（`DistributionConfig.Logging`）と v2
   （`LogDelivery` 一式）で置き場所が違う**。v2 を使っている限り legacy 側は `Enabled:false` が正常で、
   **「無効」ではなく「そこには無い」**。1つのフィールドで「無い」と言い切ったのは負の対照の欠落。
   ✅ **正しい確認は `aws logs describe-log-groups --log-group-name-prefix siko-`**
   （＝**受け手側から見る**。送り手の設定は版によって場所が変わるが、受け手にログが溜まっていれば
   届いているのは確実）。
2. **記録を読まなかった。** 本ログの 08-03 に「R-4 が初めて『原因を言える』側で働いた」と
   **自分で書いてある**。`docs/pour-over-leftovers.md` の R-4 にも引き方の手順がある。

🔑 **規則: 「観測手段が無い」と言う前に、受け手側を1回見る。**
そして **診断は「入れた」ではなく「引いた」で初めて価値になる**。
R-4 は08-03に1回引かれたきり17日間引かれず、その間に**同じログの中で2件の未知の障害が記録され続けていた**
（08-13 のスキャナ・08-17 の Instagram 失効）。**入れっぱなしの診断は、無いのとほぼ同じ値段で腐る。**

📌 **系として直すなら**: 5xx アラームが鳴ったら、Slack 通知に **そのまま貼れる Logs Insights クエリ**を
含める（＝「引く」を思い出す仕組みを検知側に置く）。人の記憶に依存させない。

### 59. 🔴 **一部を経路から外しても「外した」とは言えない — 母集団は攻撃側が決める**

08-07、Lambda@Edge の throttle（`LambdaLimitExceeded`）に対して
**「503 の 97% は `/_next/static` 配下だから、そこだけ経路から外せば十分。クォータ引き上げは不要」**
と判断した（#144）。残る3%は「`/backup/.git/config` 等を舐めたスキャナ5件・ユーザー影響なし」と数えた。

🔴 **08-13、同じ形のスキャナが 1,208 req/5分 を単一エッジに撃ち、12件の 503 を作った。**
5件と1,208件を分けたのは**こちらの設計ではなく、相手の規模**である。

🔑 **規則: 「実測で 97% を占めるから残りは無視できる」は、残りの 3% の分布が
こちらの制御下にあるときにしか成り立たない。** 静的アセットの本数は
サイトの構造で決まるので予測できるが、**スキャナが何本撃つかはこちらが決められない**。
**割合ではなく「最悪の1バーストで何本来るか」で見る。**

📌 このとき「ユーザー影響なし」と書いたのも半分しか正しくない。**503 を受けるのはスキャナだが、
枯渇した同時実行枠は共有**なので、**同じ5分に来た本物のリクエストも 503 になる**。
「誰が撃ったか」と「誰が被害を受けるか」は別。

### 60. **達成条件は「達成確率」で評価する — 発生率が分かれば「待つ」の値段が計算できる**

S-3（アラーム遷移ゼロが7日連続）は 08-05・08-13・08-20 と**3回破れた**。
そのたびに「起算を引き直して待つ」を選んできたが、**待つことの期待コストを一度も計算していなかった。**

計算すると単純だった。#144 以降の13日で発報相当のイベントは2回＝**平均6.5日に1回**。
ポアソン近似で **7日連続クリアの確率は 34%**、**期待所要は約21日**。
＝ **「あと7日待てば終わる」ではなく「平均3週間、しかも終わる保証は無い」**。

🔑 **規則: 期間で定義された終了条件は、必ず「妨害イベントの発生率」とセットで見る。**
発生率 λ が分かれば、`exp(-期間 × λ)` が達成確率で、`期間 / その確率` が期待所要になる。
**確率が低いと分かったら、待つのではなく λ を下げる（＝原因を潰す）ほうへ投資を切り替える。**

📌 **もう一つの含意**: 「起算を引き直す」は誠実な運用だが、**λ を下げないまま引き直すのは
ただのループ**。08-13 に「直してから起算し直す」を選んだ判断は正しかったが、
**そのとき直したのは監視（#166）であって発生源ではなかった**。監視を直しても λ は下がらない。

### 61. **「開いたか」だけを知りたいなら、副作用の出ない入力で測れる — 観測は "何を見せるか" だけでなく "何を残すか" も設計できる**

決済再開の確認で `POST /api/checkout` を正しいボディで叩けば、**本物の Stripe セッションが1つ生まれる**。
本番に検証用のゴミが残り、期限切れイベントまで後から飛ぶ。かといって叩かなければ「開いた」と言えない。

抜け道は、**確かめたい分岐がハンドラの何番目にあるか**を読むことだった。

```
if (!isPaymentsEnabled()) return 503   ← 確かめたいのはここ
... レート制限 ...
formData = await req.formData()        ← ここで JSON は 400
```

＝ **わざと不正なボディを送れば、閉じていれば 503・開いていれば 400** で判定できる。
Stripe には一切触れず、注文も作られず、5xx も出ない。

🔑 一般化すると **「知りたい分岐より後ろを、意図的に失敗させる」**。
教訓57 は「本番に測定リクエストを打つときは、それが**監視に何を見せるか**を先に考える」だった。
その続きがこれで、**同じ問いを"何を残すか"にも向ける**と、たいてい足跡の小さい測り方が見つかる。

📌 同じ日にもう1つ使った: webhook エンドポイントの疎通は**署名なしの POST**（400 Missing signature）で測った。
「到達する」ことだけを確かめたいので、正しい署名を作る必要はない。

---

### 62. **秘密は「値を見ずに」検証できる — 名前の存在と長さで、配線と投入を別々に確かめる**

シークレットの投入は「入れたつもり」が起きやすいのに、確認しようとすると値を出したくなる
（`sst secret list` は値を出すので使えない＝教訓36）。**値を出さずに2段階で確かめられる。**

| 確かめたいこと | 手段 | この日の実測 |
|---|---|---|
| **配線**（宣言 ≠ 配線） | `--query 'keys(Environment.Variables)'` ＝ **キー名だけ**列挙 | 3本とも存在 |
| **投入** | `--query 'Environment.Variables.<KEY>' \| wc -c` ＝ **長さだけ** | `0 / 0 / 0` → **`107 / 38 / 4`** |

🔑 **長さは前後の対照になり、しかも形の検証も兼ねる**。`whsec_` は接頭辞6＋32桁＝38、
`PAYMENTS_ENABLED=true` は4。投入前に `scripts/check-secret-file.mjs` が出した `len` と
デプロイ後の Lambda の `len` が一致すれば、**「同じ値が届いた」まで言える**。

🔴 このプロジェクトは「宣言 ≠ 配線」を **#125・#128・#160・15①** と4回踏んでいる。
5回目を避けられたのは、**投入する前にキー名だけを本番の実体に問い合わせた**から。
＝ **秘密の検証で妥協して「見ない」を選ぶ必要はない。見るべきものが値ではないだけ。**

---

### 63. **手順書は「対象が今も本番か」だけでなく「実行者が今も自分か」も腐る**

15 の計画は投入先が Vercel のまま腐っていた（runbook §1）。この日はもう1種類の腐り方が出た。
`npm run sst:deploy -- --stage production` が **auto mode の分類器にブロックされ、Claude では打てなかった**。
手順書は「私が全部やる」前提で書かれていたが、**不可逆に近い操作ほど実行者が人間に戻る**。

🔑 救われたのは **③（投入）と④（デプロイ）が別ステップで、間で止まっても停止側に倒れる**設計だったから。
値は state にあるだけ・Lambda の env は空・`isPaymentsEnabled()` は false ＝ **中断が安全**。

→ **手順を分けるときは「途中で止まったらどちら側に倒れるか」を各境界で言えるようにする。**
権限・時間切れ・人の交代のどれで止まっても、同じ問いの答えが効く。
（📌 実務上のメモ: 本番デプロイはオーナーが手で打つ。ワークツリー内で
`AWS_PROFILE=process npm run sst:deploy -- --stage production`）
