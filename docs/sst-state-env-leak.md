...
#### ✅ 検証の決着（2026-08-08〜08-09）

**回した6本のうち4本を実地で証明した。** 残る2本は**発火機会が無い**ので未検証のまま置く
（＝「確かめていない」ことを明記する。「たぶん大丈夫」とは書かない）。

| secret | 検証 |
|---|---|
| `CRON_SECRET` | ✅ ローテーション後の初回 cron が **status=200**（中継と server で値が揃っている） |
| `GOOGLE_CLIENT_SECRET` | ✅ ログイン通過 |
| `LINE_CLIENT_SECRET` | ✅ ログイン通過 |
| `SLACK_WEBHOOK_URL` | ✅ **本番のフィードバック送信で通知が到達**（下記） |
| `ORDER_TOKEN_SECRET` | ⚠️ **未検証**。注文0件で発火機会が無い（＝実害も無い） |
| `REVALIDATE_SECRET` | ⚠️ **未検証**。内部用で発火機会が無い |

🔴 **`SLACK_WEBHOOK_URL` は、確かめようとして初めて「確かめる手段が無い」と分かった。**
`notifySlack` は `r.ok` を見ず `catch {}` で握り潰しており、**値が間違っていても
Slack が 403 を返すだけで `catch` にすら入らない**＝成功と区別が付かなかった（PR #155 で修正）。
🔑 **ローテーションの本当のコストは「回すこと」ではなく「回した結果を確かめられること」。**
確認手段の無い秘密は、回した瞬間から**壊れたかどうか分からない状態**になる。
