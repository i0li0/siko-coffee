...
### ② 残っている平文の後始末（✅ **B を実施済み・2026-08-08**）
...
削除件数が合っていても目的は果たせていない。**残った38オブジェクトを全部ダウンロードして走査した。**

そこで `eventlog/` の3本に `SST_SECRET_` が **17回**出て一度は「修正後なのに平文が残った」と判断したが、
**これは誤りで、入っていたのは名前だけだった**:

| 判定材料 | pre-fix の `app/`（値が実在した版） | post-fix の `eventlog/` |
|---|---|---|
| `"ASIA` の一致（一時資格情報の値） | **4〜8件** | **0件** |
| `SST_SECRET_` の出現 ÷ 名前17本 | **2〜4回/本**（名前と値の両方） | **1回/本**（名前だけ） |

🔑 **文字列が「出るか出ないか」で判定すると、名前と値を区別できない。**
`SST_SECRET_` は変数名にも値にも現れる。**値だけが持つ特徴**（`ASIA` で始まる一時アクセスキー）を
対照に使って初めて「名前のみ」と確定できた。
...
**C. ローテーション**（⏳ **方針決定済み・2026-08-08／実施は未**）
