...
**最終更新: 2026-08-10**（作成時点。数値はすべてその日の実測）
...
npm run check:secret-file -- <file>                        # 投入前に必ず（値は表示されない）
...
`STRIPE_SECRET_KEY` は `sk_live_` 接頭辞、`STRIPE_WEBHOOK_SECRET` は `whsec_` 接頭辞を持つので
**`FORMATS` に正規表現を足せる**（#129 と同じ手）。
