...
| S-3 | **アラーム遷移ゼロが7日連続** | 🔴 **一度達成 → 同日に破れた（自作自演）**。`2026-08-13T17:55Z` の時点では 7日22時間で0件だったが、**#165 のデプロイ後の検証で打った `POST /api/checkout` の 503（3本中1本＝33.3%）が `cloudfront-5xx` を鳴らした**（ALARM `20:31:32Z` → OK `20:51:32Z`）。→ **アラームを率から件数へ直したうえで起算し直す**（オーナー判断・教訓57） |
